...
import com.hcmut.admin.utrafficsystem.tbt.NavTest;
...
public class MapView {
    ...
    public void setOnRouteChanged(BiConsumer<Pair<Float, Float>, List<Coord>> onRouteChanged) {
        nav.setOnRouteChanged(onRouteChanged);
    }

    public void setOnFinish(Runnable onFinish) {
        nav.setOnFinish(onFinish);
    }
    ...
    public Location setCurLocation(Location location) {
        return nav.setCurLocation(location);
    }
    ...
    public void setRenderLocation(double lon, double lat, float radius) {
        ...
        if (radius > 0) {
            if (curTileIds.size() == 0 && BuildConfig.DEBUG) {
                config.dbDao.clearAll();
                Log.d(TAG, "cleared db: " + NavTest.getDatabaseSizeInBytes(config.context, AppDatabase.DATABASE_NAME));
            }
            debounce.debounce(() -> {
